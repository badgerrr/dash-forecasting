...
def prepare_data():
    ...
    data['unique_visitors'] = data['unique_visitors'].replace(',', '', regex=True).astype(int)
    data['product_visitors'] = data['product_visitors'].replace(',', '', regex=True).astype(int)
    data['segment_visitors'] = data['segment_visitors'].replace(',', '', regex=True).astype(int)
...
def ratio_calculator(df, numerator, denominator, agg_date , ratio_name):
    ratio = df[[agg_date, left, right]]
    ratio[ratio_name] = ratio.apply(lambda row: row[numerator]/row[denominator], axis=1)
    return ratio

def driver_calculator(df, left, right, metric_name):
    df[metric] = df.apply(lambda row: row[left]*row[right], axis=1)
    return df

def get_forecast(df, metric, date_field, seasonality=True, period_in_days):
    m = Prophet(yearly_seasonality=seasonality)
    data = raw_data.rename(columns={date_field: 'ds', metric:'y'})[['ds','y']]
    #m.fit(data)
...
def get_baseline():
    ...
    forecast_uv = get_forecast(df, 'unique_visitors','event_date', True, 365)

    # Calculate UV PV ratio
    uv_pv_ratio = ratio_calculator(df,'product_visitors', 'unique_visitors', 'event_date', 'uv_pv')

    # Forecast UV PV Ratio
    forecast_uv_pv = get_forecast(df, 'uv_pv','event_date', True, 365)

    # Merge Drivers back together
    df_merged = pd.merge(forecast_uv, forecast_uv_pv, how='inner', on='ds')

    # Calculate output of drivers
    result = driver_calculator(df_merged, 'unique_visitors', 'uv_pv', 'product_visitors')
